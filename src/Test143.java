...
■ 문자 코드의 종류
	...
	○ EUC-KR(MS949)
	   ...
	   한글 인코딩 방식.
	   영문은 KSC5636으로 처리하고, 한글은 KSC5601
...
public class Test143
{
	...
	{
		byte[] buf;
		String name = new String("김정용");

		System.out.println("디폴트 캐릭터셋 : " + System.getProperty("file.encoding"));
		//--==> 디폴트 캐릭터셋 : MS949 (현재 시스템(os)에서의 파일 인코딩 속성)
		...
		*/
		...
		// "UTF-8" 캐릭터셋으로 바이트 배열을 반환하는 메소드
		buf = name.getBytes("utf-8");

		System.out.println("Length : " + buf.length);	
		// byte 배열로 반환하는데, 특정한 방식으로 반환하라고 하면,
		// 그 방식을 OS에서 사용할 지 안할 지 모르기 때문에 에러남
		// => 인코딩 방식이 지원되지 않았을 때 문제 생기기 때문에 가져오지 않겠다.
		//--==> error: unreported exception UnsupportedEncodingException; 
		//	           must be caught or declared to be thrown
		//==> 예외처리 해줘야 함 ! (throws UnsupportedEncodingException)
		//						   (import java.io.UnsupportedEncodingException;)
		//--==> Length : 9
